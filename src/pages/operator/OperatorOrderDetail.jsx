...
const OperatorOrderDetail = () => {
  ...
  const handleEdit = () => {
    navigate(`/operator/teklifler/${orderId}/duzenle`);
  };

  const isAssignedToCurrentUser = () => {
    return order && (order.operationPersonId === user?.id || order.fleetPersonId === user?.id);
  };
            ...
            <CardContent>
              <Box sx={{ textAlign: 'center', py: 2 }}>
                ...
                <Typography variant="body1" sx={{ textAlign: 'center', mt: 2, fontWeight: 'bold' }}>
                  Teklif Ücreti:{' '}
                  {order.estimatedPrice || order.price || order.quotePrice
